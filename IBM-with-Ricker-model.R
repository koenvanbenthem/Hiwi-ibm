...
for(r in 1:replic){
  ...
  for(t in 1:Nt){
    ...
    if(nrow(N.w)>0){ #number of offspring per female
      for(HILFE in 1:nrow(N.w)){
        
        birth<-nrow(subset(pop,pop$patch==N.w$patch[HILFE]))
        Nchild <-c(Nchild,round(rpois(1,w(birth)))) #w(a,b,N.w$trait,N.0,N.x[N.w$patch] #each female gets a random number of offspring
      }
    }
